...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .route-cell.returned .route-from,
        .route-cell.returned .route-arrow,
        .route-cell.returned .route-to { color: var(--info); }
        ...
        .modal-gallery-item { border-radius: 10px; overflow: hidden; aspect-ratio: 1; background: var(--bg-main); border: 2px solid var(--border); cursor: pointer; transition: all 0.25s; }
...
<div class="content">
    ...
    <div class="stats-container">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value" id="statReleased"><?php echo $stats['released']; ?></div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Returned</div>
            <div class="stat-value" id="statReturned"><?php echo $stats['returned']; ?></div>
        </div>
    </div>
...
<script>
    ...
    function cleanPurpose(purpose) {
        if (!purpose) return '—';
        return purpose
            .replace(/\s*\[(dest_asset_id|dest):\d+\]/g, '')
            .replace(/\s*\|?\s*From:\s*.+?→\s*To:\s*.+$/i, '')
            .trim() || '—';
    }

    function cleanReceivedBy(val) {
        if (!val) return '—';
    ...
    function renderTable() {
        ...
        pulloutData.forEach(item => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="route-cell${route.returned ? ' returned' : ''}">
        ...
        try {
            ...
            document.getElementById('assetInfo').innerHTML = `
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-value"><code style="font-size:0.9rem;">${item.serial_number || '—'}</code></div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Condition</div>
                    <div class="detail-value"><span class="badge badge-${(item.condition||'new').toLowerCase()}">${item.condition || '—'}</span></div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Asset Location</div>
                    <div class="detail-value">${item.asset_location || '—'}</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Sub-Location</div>
                    <div class="detail-value">${item.asset_sub_location || '—'}</div>
                </div>
            `;
            ...
            document.getElementById('txnInfo').innerHTML = `
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-value"><strong style="font-size:1.2rem;">${item.quantity}</strong></div>
                </div>
                <div class="route-display" style="${route.returned ? 'border-left-color:#3498db;' : ''}">
                    <div class="loc">
                        <div class="loc-label">${route.returned ? 'Returning From' : 'From (Source)'}</div>
                        <div class="loc-value">${route.from}</div>
                    </div>
                    <i class='bx bx-right-arrow-alt arrow-icon' style="${route.returned ? 'color:#3498db;' : ''}"></i>
                    <div class="loc">
                        <div class="loc-label">${route.returned ? 'Returning To' : 'To (Recipient)'}</div>
                        <div class="loc-value">${route.to}</div>
                    </div>
                </div>
                <div class="detail-item">
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-value">${formatDate(item.date_needed)}</div>
                </div>
                <div class="detail-item">
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-value">${item.released_at ? formatDateTime(item.released_at) : '—'}</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Returned At</div>
                    <div class="detail-value">${item.returned_at ? formatDateTime(item.returned_at) : '—'}</div>
                </div>
            `;
    ...
    async function updateStatus(id, newStatus) {
        try {
            ...
            const result = await res.json();
            if (result.success) {
                const label = newStatus === 'RELEASED'  ? 'RECEIVED'
