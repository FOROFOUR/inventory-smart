...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'update_step') {
        ...
        $conn->query("INSERT INTO activity_logs (user_name,action,description) VALUES ('$user_name','PREP_STEP','$desc')");
    ...
    if ($action === 'release') {
        ...
        $stmt = $conn->prepare("UPDATE pull_out_transactions SET status='RELEASED',prep_step=4,released_at=NOW(),released_by=? WHERE id=? AND status='CONFIRMED'");
        $stmt->bind_param("si", $received_by, $id);
        $stmt->execute();
        if ($stmt->affected_rows > 0) {
            $conn->query("INSERT INTO activity_logs (user_name,action,description) VALUES ('$user_name','RELEASE_PULLOUT','Pull-out #$id RELEASED by $user_name. Received by: $received_by')");
            echo json_encode(['success'=>true,'message'=>"Request #$id marked as Released."]);
        } else { echo json_encode(['success'=>false,'message'=>'Already processed.']); }
        exit();
    }
    ...
    if ($action === 'revert') {
        ...
        if ($stmt->affected_rows > 0) {
            $conn->query("INSERT INTO activity_logs (user_name,action,description) VALUES ('$user_name','REVERT_PULLOUT','Pull-out #$id reverted to PENDING by $user_name')");
...
$search = trim($_GET['search'] ?? ''); $dateFilter = trim($_GET['date'] ?? ''); $locFilter = trim($_GET['location'] ?? '');
$sql = "SELECT p.*, a.brand, a.model, a.serial_number, c.name AS category_name, sc.name AS subcategory_name
    ...
    LEFT JOIN assets a ON p.asset_id=a.id LEFT JOIN categories c ON a.category_id=c.id LEFT JOIN sub_categories sc ON a.sub_category_id=sc.id
    WHERE p.status='CONFIRMED'";
$params=[]; $types='';
if ($search!=='') { $sql.=" AND (a.brand LIKE ? OR a.model LIKE ? OR p.requested_by LIKE ? OR p.from_location LIKE ? OR p.to_location LIKE ?)"; $l="%$search%"; $params=array_merge($params,[$l,$l,$l,$l,$l]); $types.='sssss'; }
if ($dateFilter!=='') { $sql.=" AND DATE(p.created_at)=?"; $params[]=$dateFilter; $types.='s'; }
if ($locFilter!=='') { $sql.=" AND (p.from_location LIKE ? OR p.to_location LIKE ?)"; $ll=$locFilter.'%'; $params=array_merge($params,[$ll,$ll]); $types.='ss'; }
$sql.=" ORDER BY p.created_at ASC";
$stmt=$conn->prepare($sql); if ($params) $stmt->bind_param($types,...$params); $stmt->execute();
$rows=[]; $result=$stmt->get_result(); while($r=$result->fetch_assoc()) $rows[]=$r;
$totalConfirmed = $conn->query("SELECT COUNT(*) FROM pull_out_transactions WHERE status='CONFIRMED'")->fetch_row()[0];
$readyCount   = count(array_filter($rows, fn($r) => ($r['prep_step']??0) >= 4));
$notStarted   = count(array_filter($rows, fn($r) => ($r['prep_step']??0) === 0));
$locRows=$conn->query("SELECT DISTINCT TRIM(SUBSTRING_INDEX(loc,' / ',1)) AS main_loc FROM (SELECT from_location AS loc FROM pull_out_transactions WHERE from_location IS NOT NULL AND from_location!='' AND status='CONFIRMED' UNION ALL SELECT to_location AS loc FROM pull_out_transactions WHERE to_location IS NOT NULL AND to_location!='' AND status='CONFIRMED') x ORDER BY main_loc");
$locations=[]; while($lr=$locRows->fetch_assoc()){if($lr['main_loc']) $locations[]=$lr['main_loc'];}
?>
...
<style>
:root{--purple:#8e44ad;--green:#27ae60;--red:#e74c3c;--amber:#f39c12;--info:#3498db;--bg:#f4f6f9;--white:#fff;--text:#2c3e50;--muted:#7f8c8d;--border:#e8ecf0;--shadow:0 2px 12px rgba(0,0,0,.07);--radius:12px;}
...
.stat-icon.purple{background:#f0e6f6;color:var(--purple);}.stat-icon.green{background:#d5f5e3;color:var(--green);}.stat-icon.amber{background:#fef3cd;color:var(--amber);}
.stat-value{font-size:1.6rem;font-weight:700;line-height:1;}.stat-label{font-size:.75rem;color:var(--muted);margin-top:2px;text-transform:uppercase;letter-spacing:.5px;}
...
.search-wrap{position:relative;flex:1;min-width:220px;}.search-wrap input{padding-left:2.4rem;width:100%;}.search-wrap i{position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--muted);}
...
.btn-success{background:var(--green);color:white;}.btn-success:hover:not(:disabled){background:#219a52;}
.btn-danger{background:var(--red);color:white;}.btn-danger:hover:not(:disabled){background:#c0392b;}
.btn-warning{background:var(--amber);color:white;}.btn-warning:hover:not(:disabled){background:#e67e22;}
...
.req-asset{font-size:1rem;font-weight:700;color:var(--text);}.req-model{font-size:.82rem;color:var(--muted);}

/* Step progress */
.step-progress{margin:.85rem 0 1rem;}
.step-track{display:flex;align-items:center;gap:0;margin-bottom:.5rem;}
...
.step-line.done{background:var(--purple);}.step-line.ready{background:var(--green);}
...
.step-label.done{color:var(--purple);font-weight:600;}.step-label.ready{color:var(--green);font-weight:600;}
...
.prep-route .loc{flex:1;}.prep-route .loc-label{font-size:.72rem;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;font-weight:600;}.prep-route .loc-value{font-size:.9rem;font-weight:600;}
...
.prep-meta-item .mlabel{color:var(--muted);font-size:.74rem;text-transform:uppercase;letter-spacing:.4px;}.prep-meta-item .mvalue{font-weight:600;color:var(--text);}
...
.modal{background:var(--white);border-radius:16px;width:95%;max-width:440px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.25);animation:slideUp .25s ease;}
...
.modal-close{background:none;border:none;font-size:1.4rem;cursor:pointer;color:var(--muted);border-radius:6px;padding:.2rem .4rem;}.modal-close:hover{background:var(--border);}
.modal-body{padding:1.5rem;}.modal-footer{padding:1rem 1.5rem;border-top:1px solid var(--border);display:flex;gap:.75rem;justify-content:flex-end;}
.form-group{margin-bottom:1rem;}.form-group label{display:block;font-size:.8rem;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:.4rem;}
...
<div class="content">
    <div class="page-header">
        <div><h1><i class='bx bx-loader-circle'></i> Preparing</h1>
        <p>Track and update preparation steps — tap each step node to progress</p></div>
    </div>
    ...
    <div class="filters">
        ...
        <select id="locSelect"><option value="">All Locations</option><?php foreach($locations as $loc): ?><option value="<?php echo htmlspecialchars($loc); ?>" <?php echo $locFilter===$loc?'selected':''; ?>><?php echo htmlspecialchars($loc); ?></option><?php endforeach; ?></select>
    ...
    <div class="prep-card <?php echo $isReady?'step-ready':''; ?>" id="card-<?php echo $r['id']; ?>">
        ...
        <div class="step-progress">
            <div class="step-track">
                <?php for($i=1;$i<=4;$i++): $isDone=($step>=$i); $isActive=(!$isDone&&$step===$i-1); ?>
                <div class="step-node <?php echo $isDone?($i===4?'ready':'done'):($isActive?'active':''); ?>"
                     id="node-<?php echo $r['id'].'-'.$i; ?>"
                     onclick="setStep(<?php echo $r['id'].','.$i; ?>)"
                     title="<?php echo ['','Locate asset','Check condition','Pack & label','Ready to release'][$i]; ?>">
                    <?php if($isDone): ?><i class='bx bx-check' style="font-size:.9rem;"></i><?php else: echo $i; endif; ?>
                </div>
                <?php if($i<4): ?><div class="step-line <?php echo $step>=$i?($step>=4?'ready':'done'):''; ?>" id="line-<?php echo $r['id'].'-'.$i; ?>"></div><?php endif; ?>
                <?php endfor; ?>
            </div>
            <div class="step-labels">
                <span class="step-label <?php echo $step>=1?($step>=4?'ready':'done'):''; ?>">Locate</span>
                <span class="step-label <?php echo $step>=2?($step>=4?'ready':'done'):''; ?>">Check</span>
                <span class="step-label <?php echo $step>=3?($step>=4?'ready':'done'):''; ?>">Pack</span>
                <span class="step-label <?php echo $step>=4?'ready':''; ?>">Ready</span>
            </div>
            <div class="step-status-text <?php echo $isReady?'ready':''; ?>" id="steptext-<?php echo $r['id']; ?>">
                <?php echo $isReady?'✓ Ready to Release':($step===0?'Tap a step to start preparing':'Step '.$step.' / 4 — '.['','Located','Checked','Packed','Ready to Release'][$step]); ?>
            </div>
        </div>
        ...
        <div class="prep-meta">
            ...
            <?php if($purpose!=='—'): ?><div class="prep-meta-item" style="grid-column:1/-1;"><div class="mlabel">Purpose</div><div class="mvalue"><?php echo htmlspecialchars($purpose); ?></div></div><?php endif; ?>
        </div>

        <div class="prep-actions">
            <button class="btn btn-success" id="releaseBtn-<?php echo $r['id']; ?>" onclick="openRelease(<?php echo $r['id']; ?>)" <?php echo !$isReady?'disabled title="Complete all steps first"':''; ?>>
...
<div class="modal-overlay" id="releaseModal">
    <div class="modal">
        <div class="modal-header"><h3><i class='bx bx-check-circle' style="color:var(--green)"></i> Mark as Released</h3><button class="modal-close" onclick="closeModal('releaseModal')"><i class='bx bx-x'></i></button></div>
        <div class="modal-body"><input type="hidden" id="releaseId"><div class="form-group"><label>Received By</label><input type="text" id="releaseReceivedBy" placeholder="Name of person receiving the asset"></div></div>
        <div class="modal-footer"><button class="btn btn-secondary" onclick="closeModal('releaseModal')">Cancel</button><button class="btn btn-success" onclick="submitRelease()"><i class='bx bx-check'></i> Confirm Release</button></div>
    </div>
</div>
...
<div class="modal-overlay" id="cancelModal">
    <div class="modal">
        <div class="modal-header"><h3><i class='bx bx-x-circle' style="color:var(--red)"></i> Cancel Transfer</h3><button class="modal-close" onclick="closeModal('cancelModal')"><i class='bx bx-x'></i></button></div>
        <div class="modal-body"><input type="hidden" id="cancelId"><div class="form-group"><label>Reason (optional)</label><textarea id="cancelReason" rows="3" placeholder="Enter reason..."></textarea></div></div>
        <div class="modal-footer"><button class="btn btn-secondary" onclick="closeModal('cancelModal')">Back</button><button class="btn btn-danger" onclick="submitCancel()"><i class='bx bx-block'></i> Confirm Cancel</button></div>
    </div>
</div>
...
function applyFilters(){const p=new URLSearchParams({search:document.getElementById('searchInput').value,date:document.getElementById('dateInput').value,location:document.getElementById('locSelect').value});window.location.href='admin-preparing.php?'+p.toString();}
...
async function setStep(id, clickedStep){
    ...
    // Click same last step = undo it (go back one)
    const newStep = (cur === clickedStep) ? clickedStep - 1 : clickedStep;
    const body = new FormData(); body.append('action','update_step'); body.append('id',id); body.append('step',newStep);
    try {
...
function getCurrentStep(id){
    for(let i=4;i>=1;i--){const n=document.getElementById('node-'+id+'-'+i);if(n&&(n.classList.contains('done')||n.classList.contains('ready')))return i;}
    return 0;
}
...
function updateCardUI(id, step){
    ...
    // Nodes
    for(let i=1;i<=4;i++){
        const node=document.getElementById('node-'+id+'-'+i); if(!node)continue;
    ...
    // Lines
    for(let i=1;i<=3;i++){const l=document.getElementById('line-'+id+'-'+i);if(!l)continue;l.className='step-line';if(step>=i)l.classList.add(isReady?'ready':'done');}
    // Labels
    const lbls=document.querySelectorAll('#card-'+id+' .step-label');
    [1,2,3,4].forEach((s,idx)=>{if(!lbls[idx])return;lbls[idx].className='step-label';if(step>=s)lbls[idx].classList.add(isReady?'ready':'done');});
    // Status text
    ...
    // Card border
    const card=document.getElementById('card-'+id);if(card)card.classList.toggle('step-ready',isReady);
    // Release button
    const btn=document.getElementById('releaseBtn-'+id);if(btn){btn.disabled=!isReady;btn.title=isReady?'':'Complete all steps first';}
}

function openRelease(id){document.getElementById('releaseId').value=id;document.getElementById('releaseReceivedBy').value='';document.getElementById('releaseModal').classList.add('active');}
async function submitRelease(){
    const id=document.getElementById('releaseId').value; const rb=document.getElementById('releaseReceivedBy').value;
    const body=new FormData();body.append('action','release');body.append('id',id);body.append('received_by',rb);
    const res=await fetch('admin-preparing.php',{method:'POST',body}); const result=await res.json();
...
async function revertToPending(id){
    ...
    const body=new FormData();body.append('action','revert');body.append('id',id);
    const res=await fetch('admin-preparing.php',{method:'POST',body}); const result=await res.json();
    showToast(result.message,result.success?'success':'error');
    if(result.success){document.getElementById('card-'+id)?.remove();updateStat(-1);}
}

function openCancel(id){document.getElementById('cancelId').value=id;document.getElementById('cancelReason').value='';document.getElementById('cancelModal').classList.add('active');}
async function submitCancel(){
    const id=document.getElementById('cancelId').value;
    const body=new FormData();body.append('action','cancel');body.append('id',id);body.append('reason',document.getElementById('cancelReason').value);
    const res=await fetch('admin-preparing.php',{method:'POST',body}); const result=await res.json();
    showToast(result.message,result.success?'success':'error');
...
function exportCSV(){
    ...
    const data=<?php $out=[];foreach($rows as $r){$from=$r['from_location']?:'—';$to=$r['to_location']?:($r['location_received']?:'—');$p=preg_replace('/\s*\[(dest_asset_id|dest):\d+\]/','', $r['purpose']??'');$p=preg_replace('/\s*\|?\s*From:\s*.+?→\s*To:\s*.+$/i','',$p);$sl=['Not Started','Located','Checked','Packed','Ready to Release'];$out[]=[$r['id'],$r['brand']??'',$r['model']??'',$r['serial_number']??'',$r['subcategory_name']??'',$r['quantity'],$from,$to,trim($p),$r['requested_by']??'',$r['date_needed']??'',$sl[$r['prep_step']??0]??''];} echo json_encode($out); ?>;
    const csv=[headers,...data].map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n');
    const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));a.download='preparing_'+new Date().toISOString().slice(0,10)+'.csv';a.click();
}

function updateStat(d){const s=document.getElementById('statConfirmed');if(s)s.textContent=Math.max(0,parseInt(s.textContent)+d);}
