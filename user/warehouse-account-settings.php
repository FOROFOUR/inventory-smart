...
function fetchUser($conn, $userId) {
    $stmt = $conn->prepare("SELECT name, email, role, profile_pic, warehouse_location FROM users WHERE id = ?");
    ...
    if (!empty($_FILES['profile_pic']['name'])) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            if (move_uploaded_file($_FILES['profile_pic']['tmp_name'], $uploadDir . $newFilename)) {
                if (!empty($user['profile_pic'])) @unlink($uploadDir . $user['profile_pic']);
...
if (isset($_POST['change_password'])) {
    ...
    $pwStmt->bind_param("i", $userId); $pwStmt->execute();
    ...
    if (!password_verify($currentPw, $pwRow['password'])) {
        ...
        $upStmt->bind_param("si", $hash, $userId); $upStmt->execute();
...
<head>
    ...
    <title>Account Settings — Warehouse</title>
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet'>
    <style>
        /* ── Reset ── */
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #f4f6fb; font-family: 'Poppins', sans-serif; overflow-x: hidden; }

        /* ── Layout — matches warehouse sidebar ── */
        .main-content {
            position: relative;
            left: 250px;
        ...
        .page-header-icon {
            ...
            box-shadow: 0 4px 14px rgba(79,142,247,.35); flex-shrink: 0;
        }
        .page-header h1 { font-size: 1.45rem; font-weight: 700; color: #0f172a; margin-bottom: 3px; letter-spacing: -.3px; }
        .page-header p  { font-size: .82rem; color: #94a3b8; font-weight: 400; }
        ...
        .alert-success { background: #f0fdf4; color: #166534; border: 1px solid #bbf7d0; box-shadow: 0 2px 10px rgba(22,163,74,.1); }
        .alert-error   { background: #fff1f2; color: #9f1239;  border: 1px solid #fecdd3; box-shadow: 0 2px 10px rgba(244,63,94,.1); }
        @keyframes alertIn { from{opacity:0;transform:translateY(-8px) scale(.98);}to{opacity:1;transform:translateY(0) scale(1);} }

        /* ── Settings grid ── */
        .settings-grid { display: grid; grid-template-columns: 320px 1fr; gap: 26px; align-items: start; }
        ...
        .settings-card:hover { box-shadow: 0 2px 8px rgba(15,23,42,.06), 0 8px 32px rgba(15,23,42,.09); }
        .card-header {
            padding: 20px 24px; border-bottom: 1px solid #f1f5f9;
            display: flex; align-items: center; gap: 14px; background: #fafbfe;
        }
        .header-icon { width: 42px; height: 42px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.15rem; color: #fff; flex-shrink: 0; }
        ...
        .card-header h3 { font-size: .975rem; font-weight: 700; color: #0f172a; margin-bottom: 2px; letter-spacing: -.1px; }
        .card-header p  { font-size: .775rem; color: #94a3b8; font-weight: 400; }
        .card-body { padding: 26px 24px; }

        /* ── Avatar ── */
        .avatar-wrapper { text-align: center; padding-bottom: 22px; border-bottom: 1px solid #f1f5f9; margin-bottom: 22px; }
        .avatar-ring { position: relative; display: inline-block; margin-bottom: 14px; }
        .avatar-ring img {
            width: 110px; height: 110px; border-radius: 50%;
            object-fit: cover; border: 4px solid #fff;
            box-shadow: 0 0 0 3px #e8ecf3, 0 8px 28px rgba(15,23,42,.14); display: block;
        }
        .avatar-edit-btn {
            ...
            color: #fff; cursor: pointer; border: 2.5px solid #fff;
            font-size: .95rem; transition: transform .2s, box-shadow .2s;
            box-shadow: 0 3px 10px rgba(37,99,235,.4);
        }
        .avatar-edit-btn:hover { transform: scale(1.12); box-shadow: 0 5px 16px rgba(37,99,235,.5); }
        .avatar-name { font-size: 1.05rem; font-weight: 700; color: #0f172a; margin-bottom: 5px; }
        .avatar-meta { display: flex; align-items: center; justify-content: center; gap: 6px; flex-wrap: wrap; margin-top: 4px; }
        .avatar-role {
            display: inline-flex; align-items: center; gap: 5px;
            padding: 3px 12px; border-radius: 20px;
            font-size: .72rem; font-weight: 700;
            background: #eff6ff; color: #2563eb;
            text-transform: uppercase; letter-spacing: .6px; border: 1px solid #bfdbfe;
        }
        .avatar-location {
        ...
        .drop-zone {
            ...
            transition: all .25s; background: #f8fafc; display: block;
        }
        .drop-zone:hover, .drop-zone.drag-over { border-color: #4f8ef7; background: #eff6ff; }
        .drop-zone .dz-icon { font-size: 2rem; color: #cbd5e1; display: block; margin-bottom: 8px; transition: color .25s; }
        .drop-zone:hover .dz-icon { color: #4f8ef7; }
        .drop-zone .dz-title { font-size: .84rem; font-weight: 600; color: #475569; margin: 0 0 3px; }
        .drop-zone .dz-sub   { font-size: .73rem; color: #94a3b8; }
        #picPreview { display: none; max-width: 100%; max-height: 140px; border-radius: 10px; margin-top: 12px; object-fit: cover; border: 2px solid #e8ecf3; box-shadow: 0 2px 12px rgba(15,23,42,.08); }
        ...
        .form-group label {
            ...
            margin-bottom: 8px; text-transform: uppercase; letter-spacing: .55px;
        }
        .form-group label i { font-size: .9rem; color: #94a3b8; }
        .input-wrap { position: relative; }
        .input-wrap .input-icon {
            position: absolute; left: 14px; top: 50%; transform: translateY(-50%);
            color: #94a3b8; font-size: 1.05rem; pointer-events: none; z-index: 1; transition: color .2s;
        }
        ...
        .input-wrap input:focus { border-color: #4f8ef7; box-shadow: 0 0 0 3.5px rgba(79,142,247,.14); background: #fff; }
        .input-wrap input:disabled { background: #f1f5f9; color: #94a3b8; cursor: not-allowed; border-color: #e2e8f0; }
        .input-wrap input::placeholder { color: #cbd5e1; }
        .toggle-pw { position: absolute; right: 13px; top: 50%; transform: translateY(-50%); color: #94a3b8; cursor: pointer; font-size: 1.05rem; transition: color .2s; z-index: 2; }
        .toggle-pw:hover { color: #4f8ef7; }

        /* ── Password strength ── */
        .pw-strength-bar { height: 5px; border-radius: 99px; margin-top: 10px; background: #e2e8f0; overflow: hidden; }
        .pw-strength-fill { height: 100%; border-radius: 99px; width: 0; transition: width .35s ease, background .35s ease; }
        .pw-strength-row { display: flex; align-items: center; justify-content: space-between; margin-top: 6px; }
        .pw-strength-label { font-size: .72rem; font-weight: 600; color: #94a3b8; transition: color .3s; }
        .pw-strength-hint  { font-size: .7rem; color: #cbd5e1; }

        /* ── Section divider ── */
        .section-divider { height: 1px; background: linear-gradient(to right, transparent, #e2e8f0, transparent); margin: 8px 0 22px; }
        ...
        .btn-save {
            ...
            gap: 8px; transition: transform .18s, box-shadow .18s, filter .18s; letter-spacing: .1px;
        }
        .btn-save:active { transform: scale(.98); }
        .btn-primary {
            background: linear-gradient(135deg,#4f8ef7 0%,#2563eb 100%);
            color: #fff; box-shadow: 0 4px 14px rgba(37,99,235,.28); margin-top: 4px;
        }
        .btn-primary:hover { filter: brightness(1.07); box-shadow: 0 6px 22px rgba(37,99,235,.38); transform: translateY(-1px); }
        .btn-danger {
            background: linear-gradient(135deg,#f87171 0%,#dc2626 100%);
            color: #fff; box-shadow: 0 4px 14px rgba(220,38,38,.25); margin-top: 4px;
        }
        .btn-danger:hover { filter: brightness(1.07); box-shadow: 0 6px 22px rgba(220,38,38,.38); transform: translateY(-1px); }
        ...
        @media(max-width:1024px){
            .main-content{left:88px;width:calc(100% - 88px);padding:24px 24px;}
            .sidebar.close ~ .main-content{left:88px;width:calc(100% - 88px);}
            .settings-grid{grid-template-columns:280px 1fr;}
        }

        /* Tablet 860px — stack to single column */
        @media(max-width:860px){
            .main-content{left:0!important;width:100%!important;padding:18px 16px;}
            .settings-grid{grid-template-columns:1fr;}
            .form-row{grid-template-columns:1fr;gap:0;}
            .page-header{flex-direction:column;align-items:flex-start;gap:8px;margin-bottom:22px;padding-bottom:16px;}
        }
        ...
        @media(max-width:480px){
            ...
            .avatar-ring img{width:90px;height:90px;}
...
<div class="main-content">
    ...
    <?php if ($successMsg): ?>
        <div class="alert alert-success"><i class='bx bx-check-circle'></i><?= htmlspecialchars($successMsg) ?></div>
    <?php endif; ?>
    <?php if ($errorMsg): ?>
        <div class="alert alert-error"><i class='bx bx-error-circle'></i><?= htmlspecialchars($errorMsg) ?></div>
    <?php endif; ?>
            ...
            <div class="card-body">
                <div class="avatar-wrapper">
                    <div class="avatar-ring">
                        <img src="<?= htmlspecialchars($profilePic) ?>" alt="Profile" id="currentAvatar">
                    ...
                    <div class="avatar-meta">
                        <span class="avatar-role"><?= htmlspecialchars($user['role'] ?? '') ?></span>
                        <?php if (!empty($user['warehouse_location'])): ?>
                            <span class="avatar-location">
                                <i class='bx bxs-warehouse'></i>
                                <?= htmlspecialchars($user['warehouse_location']) ?>
                            </span>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <form method="POST">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                <label><i class='bx bxs-warehouse'></i> Warehouse Location</label>
                                <div class="input-wrap">
                                    <i class='bx bxs-warehouse input-icon'></i>
                                    <input type="text" value="<?= htmlspecialchars($user['warehouse_location'] ?? '—') ?>" disabled>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <div class="input-wrap">
                                    ...
                                    <input type="password" name="new_password" id="newPw" placeholder="Min. 8 characters" required>
...
picInput.addEventListener('change', function() {
    if (this.files && this.files[0]) {
        ...
        reader.onload = e => { picPreview.src = e.target.result; picPreview.style.display = 'block'; };
...
['dragover','dragleave','drop'].forEach(ev => {
    dropZone.addEventListener(ev, e => {
        ...
        if (ev === 'drop') { picInput.files = e.dataTransfer.files; picInput.dispatchEvent(new Event('change')); }
    });
});
