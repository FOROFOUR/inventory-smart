...
.main-content{
  ...
  background:#f4f6f9;padding:28px 32px;
...
.hero{
  ...
  margin-bottom:24px;box-shadow:0 8px 32px rgba(37,99,235,.25);
...
.scard{background:#fff;border-radius:16px;padding:18px 20px;border:1px solid #e8ecf3;
  box-shadow:0 1px 4px rgba(15,23,42,.04),0 4px 16px rgba(15,23,42,.05);
  display:flex;align-items:center;gap:14px;}
.scard-icon{width:44px;height:44px;border-radius:12px;display:flex;align-items:center;
  justify-content:center;font-size:1.25rem;color:#fff;flex-shrink:0;}
...
.qcard{background:#fff;border-radius:16px;padding:20px 24px;border:1px solid #e8ecf3;
  box-shadow:0 1px 4px rgba(15,23,42,.04),0 4px 16px rgba(15,23,42,.05);
...
.qcard-icon{width:48px;height:48px;border-radius:12px;display:flex;align-items:center;
...
/* ── Asset Table ── */
.asset-card{background:#fff;border-radius:18px;border:1px solid #e8ecf3;
  box-shadow:0 1px 4px rgba(15,23,42,.04),0 4px 24px rgba(15,23,42,.06);overflow:hidden;}
.toolbar{padding:14px 22px;border-bottom:1px solid #f1f5f9;background:#fafbfe;
...
.search-wrap input{width:100%;padding:8px 11px 8px 35px;border:1.5px solid #e2e8f0;
  border-radius:9px;font-family:'Poppins',sans-serif;font-size:.83rem;color:#0f172a;
  background:#f8fafc;outline:none;transition:border-color .2s;}
.search-wrap input:focus{border-color:#4f8ef7;background:#fff;}
.fsel{padding:8px 12px;border:1.5px solid #e2e8f0;border-radius:9px;
  font-family:'Poppins',sans-serif;font-size:.83rem;color:#0f172a;
  background:#f8fafc;outline:none;cursor:pointer;}
.fsel:focus{border-color:#4f8ef7;}
.table-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch;}
table{width:100%;border-collapse:collapse;}
thead{background:linear-gradient(135deg,#1e293b,#334155);}
thead th{padding:12px 16px;text-align:left;color:#fff;font-size:.74rem;font-weight:600;
...
.qty-badge{display:inline-flex;align-items:center;justify-content:center;
  ...
  background:#eff6ff;color:#2563eb;font-weight:700;font-size:.78rem;border:1px solid #bfdbfe;}
...
/* ── Toast ── */
.toast{position:fixed;top:20px;right:80px;z-index:99998;padding:12px 18px;border-radius:11px;
  font-family:'Poppins',sans-serif;font-size:.84rem;font-weight:500;
  display:flex;align-items:center;gap:9px;box-shadow:0 8px 24px rgba(15,23,42,.15);
...
@media(max-width:1024px){
  .main-content{left:88px;width:calc(100% - 88px);padding:22px 24px;}
  .sidebar.close ~ .main-content{left:88px;width:calc(100% - 88px);}
...
@media(max-width:768px){
  .main-content{left:0!important;width:100%!important;padding:16px 14px;}
  .hero{flex-direction:column;align-items:flex-start;gap:16px;padding:20px;border-radius:14px;margin-bottom:16px;}
  .hero-stats{gap:20px;width:100%;justify-content:flex-start;}
  .hero-left h1{font-size:1.2rem;}
  .hstat-val{font-size:1.6rem;}
  .cards-row{grid-template-columns:repeat(2,1fr);gap:12px;margin-bottom:16px;}
  .scard{padding:14px 16px;gap:12px;}
  .scard-icon{width:40px;height:40px;font-size:1.1rem;}
  .scard-val{font-size:1.4rem;}
  .quick-nav{grid-template-columns:1fr;gap:10px;margin-bottom:16px;}
  ...
  thead th:nth-child(4),tbody td:nth-child(4),  /* Serial */
  thead th:nth-child(8),tbody td:nth-child(8)   /* Sub-loc */
    {display:none;}
}
...
@media(max-width:480px){
  ...
  .hstat-lab{font-size:.6rem;}
  .cards-row{grid-template-columns:repeat(2,1fr);gap:8px;}
  .scard{padding:12px;gap:10px;border-radius:12px;}
  .scard-icon{width:36px;height:36px;font-size:1rem;border-radius:10px;}
  .scard-val{font-size:1.2rem;}
  .scard-label{font-size:.62rem;}
  .qcard{padding:14px 16px;gap:12px;}
  .qcard-icon{width:40px;height:40px;font-size:1.2rem;}
  .qcard-title{font-size:.9rem;}
  .qcard-label{font-size:.65rem;}
  .toolbar{flex-direction:column;align-items:stretch;}
  .search-wrap{min-width:0;}
  .fsel{width:100%;}
  /* Table — also hide Condition column */
  thead th:nth-child(6),tbody td:nth-child(6){display:none;}
  tbody td,thead th{padding:9px 10px;font-size:.78rem;}
  .toast{right:10px;left:10px;max-width:none;top:auto;bottom:16px;}
}
</style>
</head>
<body>
...
async function loadAssets() {
    try {
        ...
        document.getElementById('cTotal').textContent    = allAssets.length;
        document.getElementById('heroTotal').textContent  = allAssets.length;
        document.getElementById('cWorking').textContent   = allAssets.filter(a => a.status === 'WORKING').length;
        document.getElementById('cChecking').textContent  = allAssets.filter(a => a.status === 'FOR CHECKING').length;
        document.getElementById('cBroken').textContent    = allAssets.filter(a => a.status === 'NOT WORKING').length;
        renderAssets();
...
function renderAssets() {
    ...
    tb.innerHTML = list.map(a => `<tr>
        ...
        <td><span class="qty-badge">${a.beg_balance_count||1}</span></td>
