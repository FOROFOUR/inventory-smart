...
// Total assets (sum of quantities, not record count)
$totalAssets = $conn->query("SELECT SUM(beg_balance_count) as total FROM assets")->fetch_assoc()['total'] ?? 0;

// Working assets (sum of beg_balance_count where status = WORKING)
...
// Recent transfers (last 6)
$recentTxns = [];
$txnResult = $conn->query("SELECT p.id, p.quantity, p.purpose, p.requested_by, p.released_by, p.status, p.created_at, a.brand, a.model FROM pull_out_transactions p JOIN assets a ON p.asset_id=a.id ORDER BY p.created_at DESC LIMIT 6");
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .txn-pill .pill-val { font-size: 1.8rem; font-weight: 700; line-height: 1; }
        .txn-pill.pend   .pill-val { color: #f39c12; }
        .txn-pill.recv   .pill-val { color: var(--secondary); }
        .txn-pill.ret    .pill-val { color: var(--info); }
        .txn-pill .pill-label { font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-secondary); margin-top: 0.25rem; font-weight: 600; }
        ...
        .ls-name { flex: 1; font-size: 0.88rem; font-weight: 600; }
        .ls-loc  { font-size: 0.75rem; color: var(--text-secondary); }
        .ls-qty  { font-size: 0.8rem; font-weight: 700; background: #ffeaea; color: var(--danger); padding: 0.2rem 0.6rem; border-radius: 6px; }
        ...
        .table-card .card-header { padding: 1.25rem 1.5rem; margin-bottom: 0; border-bottom: 2px solid var(--border); border-radius: 0; }
        .view-all-btn {
            font-size: 0.82rem;
            font-weight: 600;
            color: var(--secondary);
        ...
        tbody td { padding: 0.9rem 1rem; color: var(--text-primary); white-space: nowrap; }
        ...
        .route-cell .r-arrow { font-size: 0.72rem; color: var(--secondary); font-weight: 700; }
        .route-cell .r-to { font-size: 0.85rem; font-weight: 600; }

        /* Badges */
        .badge { display: inline-block; padding: 0.28rem 0.7rem; border-radius: 6px; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.3px; font-family: 'JetBrains Mono', monospace; }
...
<div class="content">
    ...
    <div class="kpi-grid">
        <div class="kpi-card blue">
            ...
            <div class="kpi-info">
                <div class="kpi-value"><?php echo number_format($totalAssets); ?></div>
                <div class="kpi-label">Total Assets</div>
                <div class="kpi-sub"><?php echo $totalLocations; ?> locations</div>
            </div>
        </div>
        <div class="kpi-card green">
            <div class="kpi-icon"><i class='bx bx-check-shield'></i></div>
            <div class="kpi-info">
                <div class="kpi-value"><?php echo number_format($workingAssets); ?></div>
                <div class="kpi-label">Working Assets</div>
                <div class="kpi-sub"><?php echo number_format($totalAssets - $workingAssets); ?> need attention</div>
            </div>
        </div>
        <div class="kpi-card orange">
            <div class="kpi-icon"><i class='bx bx-time-five'></i></div>
            <div class="kpi-info">
                <div class="kpi-value"><?php echo number_format($statusData['FOR CHECKING'] ?? 0); ?></div>
                <div class="kpi-label">For Checking</div>
                <div class="kpi-sub"><?php echo number_format($statusData['NOT WORKING'] ?? 0); ?> not working</div>
            </div>
        </div>
        <div class="kpi-card purple">
            <div class="kpi-icon"><i class='bx bx-transfer-alt'></i></div>
            <div class="kpi-info">
                <div class="kpi-value"><?php echo number_format($txnStats['pending']); ?></div>
                <div class="kpi-label">In Transit</div>
                <div class="kpi-sub"><?php echo number_format($txnStats['total']); ?> total transfers</div>
            </div>
        </div>
    </div>
    ...
    <div class="charts-grid">
        <div class="chart-card">
            <div class="card-header">
                <h3><i class='bx bx-bar-chart-alt-2'></i> Assets by Category</h3>
            </div>
        ...
        <div class="chart-card">
            <div class="card-header">
                <h3><i class='bx bx-map-pin'></i> Top Locations</h3>
            </div>
            <div class="chart-wrap">
        ...
        <div class="chart-card">
            <div class="card-header">
                <h3><i class='bx bx-pie-chart-alt-2'></i> Asset Status</h3>
            </div>
        ...
        <div class="chart-card">
            <div class="card-header">
                <h3><i class='bx bx-doughnut-chart'></i> Condition</h3>
            </div>
            <div class="chart-wrap" style="height:200px;">
    ...
    <div class="bottom-grid">
        ...
        <div class="info-card">
            <div class="card-header">
                <h3><i class='bx bx-transfer-alt'></i> Transfer Summary</h3>
            </div>
            <div class="txn-pills">
                <div class="txn-pill total">
                    <div class="pill-val" style="color:var(--primary);"><?php echo number_format($txnStats['total']); ?></div>
                    <div class="pill-label">Total</div>
                </div>
                <div class="txn-pill pend">
                    <div class="pill-val"><?php echo number_format($txnStats['pending']); ?></div>
                    <div class="pill-label">Pending</div>
                </div>
                <div class="txn-pill recv">
                    <div class="pill-val"><?php echo number_format($txnStats['received']); ?></div>
                    <div class="pill-label">Received</div>
                </div>
                <div class="txn-pill ret">
                    <div class="pill-val"><?php echo number_format($txnStats['returned']); ?></div>
        ...
        <div class="info-card">
            <div class="card-header">
                ...
                <?php if (count($lowStock)): ?>
                    <span style="font-size:0.78rem;background:#ffeaea;color:var(--danger);padding:0.2rem 0.6rem;border-radius:6px;font-weight:700;"><?php echo count($lowStock); ?> items</span>
                <?php endif; ?>
            </div>
            <?php if (count($lowStock)): ?>
                    ...
                    <?php foreach ($lowStock as $item): ?>
                        <div class="low-stock-item">
                            ...
                            <div style="flex:1; min-width:0;">
                                <div class="ls-name"><?php echo htmlspecialchars($item['brand'] . ' ' . $item['model']); ?></div>
                                <div class="ls-loc"><i class='bx bx-map-pin' style="vertical-align:middle;font-size:0.8rem;"></i> <?php echo htmlspecialchars($item['location'] ?: '—'); ?></div>
                            </div>
                            <div class="ls-qty"><?php echo $item['beg_balance_count']; ?> left</div>
                        </div>
                    <?php endforeach; ?>
                </div>
            <?php else: ?>
        ...
        <table>
            ...
            <tbody>
                <?php foreach ($recentTxns as $txn):
                    // Parse from/to from purpose
                    $from = '—'; $to = '—'; $label = htmlspecialchars($txn['purpose']);
                    $clean = preg_replace('/\s*\[(dest_asset_id|dest):\d+\]/', '', $txn['purpose']);
                    if (preg_match('/From:\s*(.+?)\s*→\s*To:\s*(.+)$/', $clean, $m)) {
                        $from  = htmlspecialchars(trim($m[1]));
                        $to    = htmlspecialchars(trim($m[2]));
                        $label = htmlspecialchars(trim(preg_replace('/\s*\|.*$/', '', $clean)));
                    }
                    $receivedBy = trim(preg_replace('/\s*\[(dest_asset_id|dest):\d+\]/', '', $txn['released_by'] ?? ''));
                    ...
                    $badgeLabel = match($txn['status']) {
                        'RELEASED' => 'RECEIVED',
                        default    => $txn['status']
                    };
                ?>
                <tr>
                    <td style="color:var(--text-secondary);font-size:0.8rem;">#<?php echo $txn['id']; ?></td>
                    <td>
                        <div class="asset-cell">
                            <div class="a-brand"><?php echo htmlspecialchars($txn['brand']); ?></div>
                            <div class="a-model"><?php echo htmlspecialchars($txn['model']); ?></div>
                        </div>
                    </td>
                    <td><strong><?php echo $txn['quantity']; ?></strong></td>
                    <td>
                        <div class="route-cell">
                            <div class="r-from"><?php echo $from; ?></div>
                            <div class="r-arrow">↓</div>
                            <div class="r-to"><?php echo $to; ?></div>
                        </div>
                    </td>
                    <td><?php echo htmlspecialchars($txn['requested_by']); ?></td>
                    <td><?php echo htmlspecialchars($receivedBy ?: '—'); ?></td>
                    <td style="color:var(--text-secondary);font-size:0.82rem;"><?php echo date('M d, Y', strtotime($txn['created_at'])); ?></td>
...
new Chart(document.getElementById('categoryChart'), {
    ...
    data: {
        ...
        datasets: [{
            ...
            backgroundColor: [
                'rgba(9,132,227,0.75)','rgba(0,184,148,0.75)','rgba(108,92,231,0.75)',
                'rgba(225,112,85,0.75)','rgba(253,203,110,0.75)','rgba(116,185,255,0.75)',
                'rgba(214,48,49,0.75)','rgba(99,110,114,0.75)'
            ],
            borderRadius: 6,
            borderSkipped: false,
        }]
...
new Chart(document.getElementById('locationChart'), {
    ...
    data: {
        ...
        datasets: [{
            label: 'Assets',
            data: <?php echo json_encode(array_column($locationData, 'count')); ?>,
...
new Chart(document.getElementById('statusChart'), {
    ...
    data: {
        ...
        datasets: [{
            data: [
                <?php echo $statusData['WORKING'] ?? 0; ?>,
                <?php echo $statusData['FOR CHECKING'] ?? 0; ?>,
                <?php echo $statusData['NOT WORKING'] ?? 0; ?>
            ],
...
new Chart(document.getElementById('conditionChart'), {
    ...
    data: {
        ...
        datasets: [{
            data: [
                <?php echo $conditionData['NEW'] ?? 0; ?>,
                <?php echo $conditionData['USED'] ?? 0; ?>
            ],
